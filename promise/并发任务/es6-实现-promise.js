const Promise = (() => {
    ...
    const PromiveValue = Symbol('PromiseValue'); // 状态数据
    const PromiseStatus = Symbol('PromiseStatus');
    const thenables = Symbol('thenables'); //thenable
    const catchables = Symbol('catchbles'); //catchables
    const changeStatus = Symbol('changeStatus');// 改变状态
    const settleHandle = Symbol('settleHandle'); //后续处理的通用函数
    const linkPromise = Symbol('linkPromise');  //创建串联的Promise
