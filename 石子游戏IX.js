...
/**
 * 假设两位玩家均采用 最佳 决策, 不能随机取
 * 被3整除
 *   奇数次: Alices 输  false
 *   偶数次: Alices 赢  true
 * 不被3整除: 数组长度
 *   奇数: 最后一个是Alices 输 false
 *   偶数: 最后一个不是Alices 赢  true
 * 不能随机取: 确保先取所有都不构成被3整除的数和
 * 
 *   
 * 
 * @param {number[]} stones 
 * @return {boolean}
 */
// 由于玩家的目标是使得已经被移除的石子的价值总和不是 33 的倍数，因此我们可以把石子分成三类，
// 它们的价值除以 33 的余数分别为 0, 1, 20,1,2。我们可以直接用 0, 1, 20,1,2 代表它们的价值，
// 对应的石子数量分别为 cnt0, cnt1, cnt}2
 
//  可以发现，移除类型 0 的石子并不会对总和产生影响，因此类型 0 的石子可以看成是「先后手」交换。
// 具体地，例如当前是 Alice 在进行操作，它发现如果自己选择移除类型 1 或 2 的石子，那么她在最后一定不能获胜。
// 这时它就可以选择移除一个类型 0 的石子，将同样的局面交给 Bob。
// 如果类型 0 的石子还没有移除完，那么 Bob 同样可以通过移除一个类型 0 的石子将局面重新交给 Alice。
// 这样不断地往复下去，我们可以得到结论：
 
//  如果类型 0 的石子的个数为偶数，那么胜负情况等价于没有类型 0 的石子的胜负情况；
//  如果类型 0 的石子个数为奇数，那么胜负情况等价于只有 1 个类型 0 的石子的胜负情况。
// 注意这里不能单纯地等价于「没有类型 0 的石子的胜负情况」的相反情况，这是因为如果所有的石子都被移除完，
// 无论谁移除了最后一个石子，都算 Alice 输。因此如果 Alice 发现自己选择移除类型 1 或 2 的石子不能获胜，
// 于是选择移除类型 0 的石子，并且它不能获胜的原因是「石子会移除完」，那么 Alice 仍然会输。
